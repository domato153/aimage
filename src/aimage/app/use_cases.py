...
class ImageEngine:
    def __init__(
        ...
        *,
        ...
        acceptable_provider_evidence: tuple[EvidenceKind, ...] = (EvidenceKind.REPRODUCED,),
    ) -> None:
        ...
        *,
        ...
        if spatial_profile is not None:
            ...
            spatial_payload.update(
                {
                    ...
                    "semantic_revision": approved.semantic_revision,
                    "created_from": (spatial_profile.object_id,),
                }
            )
            ...
            intent_payload.update(
                {
                    "object_id": new_object_id(),
                    "created_from": (approved.object_id,),
                    "spatial_profile_ref": approved_spatial.object_id,
                }
            )
        ...
        self.metadata.save_decision(intent.job_id, decision)
        baseline = next(
            baseline
            for baseline in approved.baselines
            if baseline.approval_decision_id == decision.decision_id
        )
        self.metadata.save_baseline(intent.job_id, baseline)
    ...
    async def render(
        ...
        *,
        ...
        requirement_set = derive_requirements(
            render_spec,
            operation=operation,
            acceptable_evidence_kinds=self.acceptable_provider_evidence,
        )
        ...
        validate_lowering_legality(render_spec, requirement_set, lowered.execution_plan)
        ...
        run_id = new_object_id()
        artifact_id = new_object_id()
        self.metadata.start_run(
            run_id=run_id,
            job_id=intent.job_id,
            semantic_revision=intent.semantic_revision,
            source_intent_digest=current_digest,
            execution_plan_ref=lowered.execution_plan.execution_plan_id,
            capability_descriptor_ref=descriptor.descriptor_id,
            target_model_version=descriptor.target_version_or_model,
        )
        try:
            ...
            self.metadata.finish_run(
                ...
                capability_descriptor_ref=descriptor.descriptor_id,
                status="failed",
                normalized_error_class=evidence.exception_type,
        ...
        # Late results remain durable run/artifact evidence but cannot enter current validation/acceptance.
