...
def replace_reference_binding(intent: VisualIntentState, decision: DecisionRecord, *, binding_id: str, new_artifact_ref: str) -> VisualIntentState:
    """Replace one typed reference without altering unrelated roles/dimensions."""
    ...
    next_revision = intent.semantic_revision + 1
    replacement = ReferenceBinding(binding_id=new_object_id(), artifact_ref=new_artifact_ref, role=target.role, scope_paths=target.scope_paths, dimensions=target.dimensions, authority_id=target.authority_id)
    revised = tuple(replacement if binding.binding_id == binding_id else binding for binding in intent.reference_bindings)
    return _rebuild(intent, semantic_revision=next_revision, reference_bindings=revised)
