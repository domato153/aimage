...
from aimage.contracts.resolution import Capability, ProviderCapabilityDescriptor, RequirementWiring, ResolutionResult, ResolutionStatus
...
def _size_matches(requirement: CapabilityRequirement, capability: Capability) -> bool:
    ...
    if width % divisor or height % divisor:
        return False
    aspect = width / height
    return float(limits.get("min_aspect", aspect)) <= aspect <= float(limits.get("max_aspect", aspect))
...
def _constraint_matches(key: str, expected: Any, descriptor: ProviderCapabilityDescriptor, capability: Capability) -> bool:
    ...
    if key == "evidence_kind":
        allowed = expected if isinstance(expected, (tuple, list, set, frozenset)) else (expected,)
        return descriptor.evidence_kind.value in allowed
...
def capability_satisfies(requirement: CapabilityRequirement, descriptor: ProviderCapabilityDescriptor, capability: Capability) -> bool:
...
def resolve_requirements(requirement_set: RequirementSet, descriptors: tuple[ProviderCapabilityDescriptor, ...], *, now: datetime | None = None) -> ResolutionResult:
    ...
    for requirement in requirement_set.requirements:
        ...
        if match:
            ...
            wiring.append(RequirementWiring(requirement_id=requirement.requirement_id, target_descriptor_ref=descriptor.descriptor_id, capability_name=capability.name))
    ...
    if unsatisfied_mandatory and stale_candidate_seen:
        status, rationale = ResolutionStatus.STALE_CAPABILITIES, "mandatory requirements match only stale capability evidence"
    elif unsatisfied_mandatory:
        status, rationale = ResolutionStatus.UNSATISFIED, "one or more mandatory requirements are unsatisfied"
    else:
        status, rationale = ResolutionStatus.RESOLVED, "all mandatory requirements are wired to fresh compatible capabilities"
    return ResolutionResult(job_id=requirement_set.job_id, semantic_revision=requirement_set.semantic_revision, created_from=(requirement_set.object_id,) + tuple(d.object_id for d in descriptors), render_spec_ref=requirement_set.render_spec_ref, requirement_set_ref=requirement_set.requirement_set_id, status=status, selected_strategy=tuple(used_targets), requirement_wiring=tuple(wiring), unsatisfied_mandatory=tuple(unsatisfied_mandatory), unsatisfied_optional=tuple(unsatisfied_optional), rationale=rationale)
